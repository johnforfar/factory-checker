#!/usr/bin/env python3
"""
Parallel screenshot processor using Cursor v2 MCP browser tools.
This script generates batches and coordinates parallel browser tool calls efficiently.
Optimized for faster processing with reduced wait times.
"""
import json
import os
import sys
...
# Configuration - optimized wait times
PAGE_LOAD_WAIT = 2.0  # Reduced from 3 seconds - pages usually load faster
DYNAMIC_CONTENT_WAIT = 1.0  # Additional wait for dynamic content
BATCH_SIZE = 6  # Process 6 apps in parallel (optimal for browser tools)
...
def generate_batch_instructions(batch):
    ...
    print(f"\n{'='*70}")
    print(f"BATCH PROCESSING INSTRUCTIONS")
    print(f"{'='*70}")
    print(f"Batch size: {len(batch)} apps")
    print(f"Wait time: {PAGE_LOAD_WAIT}s (reduced from 3s)")
    print(f"{'='*70}\n")
    ...
    print(f"2. Wait {PAGE_LOAD_WAIT} seconds for pages to load (reduced wait time)")
    print("3. Take screenshots for all apps in parallel using browser_take_screenshot")
    print("4. Save screenshots using save_screenshot_from_temp()")
    print("5. Create thumbnails automatically")
    print("6. Update state file")
    print(f"{'='*70}\n")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Parallel browser screenshot processor using Cursor v2 MCP tools')
    ...
    print(f"\n{'='*70}")
    print(f"CURSOR V2 BROWSER TOOLS - PARALLEL SCREENSHOT PROCESSOR")
    print(f"{'='*70}")
    print(f"Total apps needing screenshots: {len(apps)}")
    print(f"Batch size: {args.batch_size}")
    print(f"Including pending: {not args.no_pending}")
    print(f"Optimized wait time: {PAGE_LOAD_WAIT}s (reduced from 3s)")
    print(f"{'='*70}\n")
